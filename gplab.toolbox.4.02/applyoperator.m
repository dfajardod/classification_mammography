...
if opnum==0
   ...
   if params.opeq
       [validinds,state]=validateinds(pop(parentindices),pop,params,state,data,parentindices,opnum);
   else
       validinds=pop(parentindices);
       % assuming that only in opeq we risk not accepting an individual copied from the previous generation,
       % which is not necessarily true (heavydyndepth could also reject reproductions once the limit is lowered)
   end
else
   % genetic operator
   newinds=feval(params.operatornames{opnum},pop,params,state,data,parentindices);
